...
describe('<HeaderNav/>', () => {
  ...
  it("should open the menu with options for picking the cipher when user clicks on 'Ciphers' button", async () => {
    ...
    expect(screen.getByRole('menuitem', { name: /caesar/i })).toBeInTheDocument();
    expect(screen.getByRole('menuitem', { name: /vigenère/i })).toBeInTheDocument();
    expect(screen.getByRole('menuitem', { name: /emoji/i })).toBeInTheDocument();
