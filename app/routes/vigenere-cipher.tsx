...
export default function VigenereCipher() {
  const [cryptoMode, setCryptoMode] = useState<'encrypt' | 'decrypt'>(CRYPTO_MODES[0].value);
  ...
  const {
    ...
    encryptDecrypt: (value) => (cryptoMode === 'encrypt' ? vigenereEncrypt(secretKey, value) : vigenereDecrypt(secretKey, value)),
  });
  const isDesktop = useMediaQuery('(min-width:37.5em)');

  return (
    <>
      <PageDescription title={PAGES.ciphers[1].label} description={PAGES.ciphers[1].description} icon={PAGES.ciphers[1].icon} />
      <Box sx={{ marginBottom: 2 }}>
        <CryptoMode
          value={cryptoMode}
          onChange={(value) => {
            handleSetTranslatedValue(value === 'encrypt' ? vigenereEncrypt(secretKey, inputValue) : vigenereDecrypt(secretKey, inputValue));
            setCryptoMode(value);
          }}
        />
        <TextField
