...
export async function clientLoader() {
  const data = await fetch('https://raw.githubusercontent.com/bbtools-ps/emoji-cipher/main/emoji-dict/dict.json');
...
export default function EmojiCipher({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <>
      ...
      {data && (
        <>
          <PageDescription title={PAGES.ciphers[2].label} description={PAGES.ciphers[2].description} icon={PAGES.ciphers[2].icon} />
          <InputOutput inputValue={value} outputValue={translatedValue} onInputChange={handleChange} onReset={handleReset} />
        </>
      )}
      {/* Status: Rejected */}
